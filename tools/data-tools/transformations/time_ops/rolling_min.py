"""Rolling min tool - compute moving minimum."""
...
class RollingMinInput(BaseModel):
    dataset_ref: str = Field(description="Dataset reference from previous operation")
    column: str = Field(description="Numeric column to compute rolling min on")
    ...
    output_column: Optional[str] = Field(
        default=None, description="Output column name. Defaults to '{column}_rmin_{window}'"
    )


@tool(args_schema=RollingMinInput)
def rolling_min_tool(
    dataset_ref: str,
    column: str,
    window: int,
    ...
    """Compute rolling minimum over a window. Useful for trailing lows, support levels."""
    try:
        ...
        result = df.copy()
        out_col = output_column or f"{column}_rmin_{window}"
        ...
        if partition_by:
            ...
            result[out_col] = result.groupby(part_cols)[column].transform(
                lambda x: x.rolling(window=window, min_periods=effective_min_periods).min()
            )
        else:
            result[out_col] = result[column].rolling(window=window, min_periods=effective_min_periods).min()
        ...
        return format_result(ref, result, "rolling_min", detail, warnings if warnings else None)
