"""Rolling mean tool - compute moving average."""
...
class RollingMeanInput(BaseModel):
    dataset_ref: str = Field(description="Dataset reference from previous operation")
    column: str = Field(description="Numeric column to compute rolling mean on")
    window: int = Field(description="Window size (number of rows)")
    partition_by: Optional[Union[str, list[str]]] = Field(
        default=None,
        description="Column(s) to partition by. Computes rolling mean within each group."
    )
    order_by: Optional[Union[str, list[str]]] = Field(
        default=None,
        description="Column(s) to order by before computing rolling mean"
    )
    ...
    output_column: Optional[str] = Field(
        default=None,
        description="Name for output column. Defaults to '{column}_rmean_{window}'"
    )


@tool(args_schema=RollingMeanInput)
def rolling_mean_tool(
    ...
    """
    Compute rolling mean (moving average) over a window of rows.
    Useful for trend smoothing, baseline comparisons.
    ...
    try:
        ...
        if not pd.api.types.is_numeric_dtype(df[column]):
            return f"✗ Column '{column}' is not numeric (dtype: {df[column].dtype}). Rolling mean requires numeric data."
        
        result = df.copy()
        out_col = output_column or f"{column}_rmean_{window}"
        ...
        if partition_by:
            ...
            result[out_col] = result.groupby(part_cols)[column].transform(
                lambda x: x.rolling(window=window, min_periods=effective_min_periods).mean()
            )
        else:
            result[out_col] = result[column].rolling(window=window, min_periods=effective_min_periods).mean()
        ...
        ref = save_result(result, "rmn")
        detail = f"{column} → {out_col} (window={window})"
        if partition_by:
            detail += f", by={partition_by}"
        return format_result(ref, result, "rolling_mean", detail, warnings if warnings else None)
        
    except Exception as e:
        return f"✗ rolling_mean failed: {e}"
